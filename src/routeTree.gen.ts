/* eslint-disable */

// @ts-nocheck

// noinspection JSUnusedGlobalSymbols

// This file was automatically generated by TanStack Router.
// You should NOT make any changes in this file as it will be overwritten.
// Additionally, you should also exclude this file from your linter and/or formatter to prevent it from being checked or modified.

import { createFileRoute } from '@tanstack/react-router'

// Import Routes

import { Route as rootRoute } from './routes/__root'

// Create Virtual Routes

const IndexLazyImport = createFileRoute('/')()
const VolumesIndexLazyImport = createFileRoute('/volumes/')()
const NetworksIndexLazyImport = createFileRoute('/networks/')()
const ImagesIndexLazyImport = createFileRoute('/images/')()
const ContainersIndexLazyImport = createFileRoute('/containers/')()
const VolumesNameLazyImport = createFileRoute('/volumes/$name')()
const NetworksNameLazyImport = createFileRoute('/networks/$name')()
const ImagesNameLazyImport = createFileRoute('/images/$name')()
const ContainersNameLazyImport = createFileRoute('/containers/$name')()
const ImagesHubNameLazyImport = createFileRoute('/images/hub/$name')()

// Create/Update Routes

const IndexLazyRoute = IndexLazyImport.update({
  id: '/',
  path: '/',
  getParentRoute: () => rootRoute,
} as any).lazy(() => import('./routes/index.lazy').then((d) => d.Route))

const VolumesIndexLazyRoute = VolumesIndexLazyImport.update({
  id: '/volumes/',
  path: '/volumes/',
  getParentRoute: () => rootRoute,
} as any).lazy(() => import('./routes/volumes/index.lazy').then((d) => d.Route))

const NetworksIndexLazyRoute = NetworksIndexLazyImport.update({
  id: '/networks/',
  path: '/networks/',
  getParentRoute: () => rootRoute,
} as any).lazy(() =>
  import('./routes/networks/index.lazy').then((d) => d.Route),
)

const ImagesIndexLazyRoute = ImagesIndexLazyImport.update({
  id: '/images/',
  path: '/images/',
  getParentRoute: () => rootRoute,
} as any).lazy(() => import('./routes/images/index.lazy').then((d) => d.Route))

const ContainersIndexLazyRoute = ContainersIndexLazyImport.update({
  id: '/containers/',
  path: '/containers/',
  getParentRoute: () => rootRoute,
} as any).lazy(() =>
  import('./routes/containers/index.lazy').then((d) => d.Route),
)

const VolumesNameLazyRoute = VolumesNameLazyImport.update({
  id: '/volumes/$name',
  path: '/volumes/$name',
  getParentRoute: () => rootRoute,
} as any).lazy(() => import('./routes/volumes/$name.lazy').then((d) => d.Route))

const NetworksNameLazyRoute = NetworksNameLazyImport.update({
  id: '/networks/$name',
  path: '/networks/$name',
  getParentRoute: () => rootRoute,
} as any).lazy(() =>
  import('./routes/networks/$name.lazy').then((d) => d.Route),
)

const ImagesNameLazyRoute = ImagesNameLazyImport.update({
  id: '/images/$name',
  path: '/images/$name',
  getParentRoute: () => rootRoute,
} as any).lazy(() => import('./routes/images/$name.lazy').then((d) => d.Route))

const ContainersNameLazyRoute = ContainersNameLazyImport.update({
  id: '/containers/$name',
  path: '/containers/$name',
  getParentRoute: () => rootRoute,
} as any).lazy(() =>
  import('./routes/containers/$name.lazy').then((d) => d.Route),
)

const ImagesHubNameLazyRoute = ImagesHubNameLazyImport.update({
  id: '/images/hub/$name',
  path: '/images/hub/$name',
  getParentRoute: () => rootRoute,
} as any).lazy(() =>
  import('./routes/images/hub/$name.lazy').then((d) => d.Route),
)

// Populate the FileRoutesByPath interface

declare module '@tanstack/react-router' {
  interface FileRoutesByPath {
    '/': {
      id: '/'
      path: '/'
      fullPath: '/'
      preLoaderRoute: typeof IndexLazyImport
      parentRoute: typeof rootRoute
    }
    '/containers/$name': {
      id: '/containers/$name'
      path: '/containers/$name'
      fullPath: '/containers/$name'
      preLoaderRoute: typeof ContainersNameLazyImport
      parentRoute: typeof rootRoute
    }
    '/images/$name': {
      id: '/images/$name'
      path: '/images/$name'
      fullPath: '/images/$name'
      preLoaderRoute: typeof ImagesNameLazyImport
      parentRoute: typeof rootRoute
    }
    '/networks/$name': {
      id: '/networks/$name'
      path: '/networks/$name'
      fullPath: '/networks/$name'
      preLoaderRoute: typeof NetworksNameLazyImport
      parentRoute: typeof rootRoute
    }
    '/volumes/$name': {
      id: '/volumes/$name'
      path: '/volumes/$name'
      fullPath: '/volumes/$name'
      preLoaderRoute: typeof VolumesNameLazyImport
      parentRoute: typeof rootRoute
    }
    '/containers/': {
      id: '/containers/'
      path: '/containers'
      fullPath: '/containers'
      preLoaderRoute: typeof ContainersIndexLazyImport
      parentRoute: typeof rootRoute
    }
    '/images/': {
      id: '/images/'
      path: '/images'
      fullPath: '/images'
      preLoaderRoute: typeof ImagesIndexLazyImport
      parentRoute: typeof rootRoute
    }
    '/networks/': {
      id: '/networks/'
      path: '/networks'
      fullPath: '/networks'
      preLoaderRoute: typeof NetworksIndexLazyImport
      parentRoute: typeof rootRoute
    }
    '/volumes/': {
      id: '/volumes/'
      path: '/volumes'
      fullPath: '/volumes'
      preLoaderRoute: typeof VolumesIndexLazyImport
      parentRoute: typeof rootRoute
    }
    '/images/hub/$name': {
      id: '/images/hub/$name'
      path: '/images/hub/$name'
      fullPath: '/images/hub/$name'
      preLoaderRoute: typeof ImagesHubNameLazyImport
      parentRoute: typeof rootRoute
    }
  }
}

// Create and export the route tree

export interface FileRoutesByFullPath {
  '/': typeof IndexLazyRoute
  '/containers/$name': typeof ContainersNameLazyRoute
  '/images/$name': typeof ImagesNameLazyRoute
  '/networks/$name': typeof NetworksNameLazyRoute
  '/volumes/$name': typeof VolumesNameLazyRoute
  '/containers': typeof ContainersIndexLazyRoute
  '/images': typeof ImagesIndexLazyRoute
  '/networks': typeof NetworksIndexLazyRoute
  '/volumes': typeof VolumesIndexLazyRoute
  '/images/hub/$name': typeof ImagesHubNameLazyRoute
}

export interface FileRoutesByTo {
  '/': typeof IndexLazyRoute
  '/containers/$name': typeof ContainersNameLazyRoute
  '/images/$name': typeof ImagesNameLazyRoute
  '/networks/$name': typeof NetworksNameLazyRoute
  '/volumes/$name': typeof VolumesNameLazyRoute
  '/containers': typeof ContainersIndexLazyRoute
  '/images': typeof ImagesIndexLazyRoute
  '/networks': typeof NetworksIndexLazyRoute
  '/volumes': typeof VolumesIndexLazyRoute
  '/images/hub/$name': typeof ImagesHubNameLazyRoute
}

export interface FileRoutesById {
  __root__: typeof rootRoute
  '/': typeof IndexLazyRoute
  '/containers/$name': typeof ContainersNameLazyRoute
  '/images/$name': typeof ImagesNameLazyRoute
  '/networks/$name': typeof NetworksNameLazyRoute
  '/volumes/$name': typeof VolumesNameLazyRoute
  '/containers/': typeof ContainersIndexLazyRoute
  '/images/': typeof ImagesIndexLazyRoute
  '/networks/': typeof NetworksIndexLazyRoute
  '/volumes/': typeof VolumesIndexLazyRoute
  '/images/hub/$name': typeof ImagesHubNameLazyRoute
}

export interface FileRouteTypes {
  fileRoutesByFullPath: FileRoutesByFullPath
  fullPaths:
    | '/'
    | '/containers/$name'
    | '/images/$name'
    | '/networks/$name'
    | '/volumes/$name'
    | '/containers'
    | '/images'
    | '/networks'
    | '/volumes'
    | '/images/hub/$name'
  fileRoutesByTo: FileRoutesByTo
  to:
    | '/'
    | '/containers/$name'
    | '/images/$name'
    | '/networks/$name'
    | '/volumes/$name'
    | '/containers'
    | '/images'
    | '/networks'
    | '/volumes'
    | '/images/hub/$name'
  id:
    | '__root__'
    | '/'
    | '/containers/$name'
    | '/images/$name'
    | '/networks/$name'
    | '/volumes/$name'
    | '/containers/'
    | '/images/'
    | '/networks/'
    | '/volumes/'
    | '/images/hub/$name'
  fileRoutesById: FileRoutesById
}

export interface RootRouteChildren {
  IndexLazyRoute: typeof IndexLazyRoute
  ContainersNameLazyRoute: typeof ContainersNameLazyRoute
  ImagesNameLazyRoute: typeof ImagesNameLazyRoute
  NetworksNameLazyRoute: typeof NetworksNameLazyRoute
  VolumesNameLazyRoute: typeof VolumesNameLazyRoute
  ContainersIndexLazyRoute: typeof ContainersIndexLazyRoute
  ImagesIndexLazyRoute: typeof ImagesIndexLazyRoute
  NetworksIndexLazyRoute: typeof NetworksIndexLazyRoute
  VolumesIndexLazyRoute: typeof VolumesIndexLazyRoute
  ImagesHubNameLazyRoute: typeof ImagesHubNameLazyRoute
}

const rootRouteChildren: RootRouteChildren = {
  IndexLazyRoute: IndexLazyRoute,
  ContainersNameLazyRoute: ContainersNameLazyRoute,
  ImagesNameLazyRoute: ImagesNameLazyRoute,
  NetworksNameLazyRoute: NetworksNameLazyRoute,
  VolumesNameLazyRoute: VolumesNameLazyRoute,
  ContainersIndexLazyRoute: ContainersIndexLazyRoute,
  ImagesIndexLazyRoute: ImagesIndexLazyRoute,
  NetworksIndexLazyRoute: NetworksIndexLazyRoute,
  VolumesIndexLazyRoute: VolumesIndexLazyRoute,
  ImagesHubNameLazyRoute: ImagesHubNameLazyRoute,
}

export const routeTree = rootRoute
  ._addFileChildren(rootRouteChildren)
  ._addFileTypes<FileRouteTypes>()

/* ROUTE_MANIFEST_START
{
  "routes": {
    "__root__": {
      "filePath": "__root.tsx",
      "children": [
        "/",
        "/containers/$name",
        "/images/$name",
        "/networks/$name",
        "/volumes/$name",
        "/containers/",
        "/images/",
        "/networks/",
        "/volumes/",
        "/images/hub/$name"
      ]
    },
    "/": {
      "filePath": "index.lazy.tsx"
    },
    "/containers/$name": {
      "filePath": "containers/$name.lazy.tsx"
    },
    "/images/$name": {
      "filePath": "images/$name.lazy.tsx"
    },
    "/networks/$name": {
      "filePath": "networks/$name.lazy.tsx"
    },
    "/volumes/$name": {
      "filePath": "volumes/$name.lazy.tsx"
    },
    "/containers/": {
      "filePath": "containers/index.lazy.tsx"
    },
    "/images/": {
      "filePath": "images/index.lazy.tsx"
    },
    "/networks/": {
      "filePath": "networks/index.lazy.tsx"
    },
    "/volumes/": {
      "filePath": "volumes/index.lazy.tsx"
    },
    "/images/hub/$name": {
      "filePath": "images/hub/$name.lazy.tsx"
    }
  }
}
ROUTE_MANIFEST_END */
